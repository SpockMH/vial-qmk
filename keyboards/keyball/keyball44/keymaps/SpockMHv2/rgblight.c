...
#define RGBLIGHT_DEFAULT_MODE RGBLIGHT_MODE_MOUSEMOVE
...
static splash_state_t splash = { false };
...
void eeconfig_update_rgblight_default(void) {
    ...
    rgblight_config.rainbow_mode = true;
...
///////////////////////////////////////////////////////////////////////////////
// Splash Wave (Smooth Fixed-Point version)
//
// 【今回の進化ポイント】
// ・距離を10倍スケール化（DIST_SCALE）
// ・radius を accumulation で連続化
// ・diff を割合計算に変更
// ・完全サブピクセル化で超スムーズ
///////////////////////////////////////////////////////////////////////////////

#define DIST_SCALE 10            // ← 1キー=10単位（超重要）
#define WAVE_SOLID 100  // -10キーまで255固定
#define WAVE_FADE  150  // 減衰範囲

static inline uint8_t keyball_distance( uint8_t r1, uint8_t c1,
...
void rgblight_value(uint8_t row, uint8_t col, bool update, bool scr,bool splash_trig){
    ...
    if (splash_trig) {
        splash.active     = true;
        splash.center_row = row;
        splash.center_col = col;
        splash.start_hue  = hue_value[last_index];
        radius = 0;
    }
        ...
        if (radius > DIST_SCALE * 25) {
            ...
            for (uint8_t r = 0; r < MATRIX_ROWS; r++) {
                for (uint8_t c = 0; c < MATRIX_COLS; c++) {
                    ...
                    uint8_t led = led_index[r][c];
                    if (led == 59) continue;

                    uint16_t d =
                        keyball_distance(r, c,
                                            splash.center_row,
                                            splash.center_col);

                    int16_t diff = d - radius + 10;

                    uint8_t v = rgblight_config.val;
                    uint8_t h = splash.start_hue;
...
void underled(bool on, uint8_t hue, uint8_t sat) {
    ...
    for (uint8_t i = 20; i < 40; i++) {
        //if (rgblight_config.rainbow_mode) {
            sethsv((i-20)*13 + cur_hue, 255, val, (rgb_led_t *)&led[i]);
            
        //} else {
        //    sethsv(hue, sat, val, (rgb_led_t *)&led[i]);
        //}
    }
    cur_hue++;
}
...
void rgblight_effect_heatmap(void) {
    if (rgblight_config.enable) {
        ...
        for (i = 0; i < RGBLED_NUM; i++) {
            ...
            sethsv(hue, 255, val, (rgb_led_t *)&led[i]);
        }
...
void rgblight_effect_icewave(void) {
    ...
    for (uint8_t i = 0; i < RGBLED_NUM; i++) {
        ...
        sethsv(rgblight_config.hue, sat, rgblight_config.val, (rgb_led_t *)&led[i]);
    }
    underled(false,0,0);
    rgblight_set();
...
void rgblight_effect_mousemove(void) {
    ...
    for (uint8_t i = 0; i < RGBLED_NUM; i++) {
        //if(rgblight_config.rainbow_mode) {
            sethsv_raw(hue_value[i], 255, value[i], (rgb_led_t *)&led[i]);
        //}else{
        //    sethsv_raw(rgblight_config.hue, hue_value[i], value[i], (rgb_led_t *)&led[i]); 
        //}
    }
    underled(true,0,0);
    rgblight_set();
    rgblight_value(0,0,false,false,false); //軌跡削除処理
}
...
void rgblight_effect_scrollmove(void) {
    ...
    for (uint8_t i = 0; i < RGBLED_NUM; i++) {
        ...
        //if(rgblight_config.rainbow_mode) {
            sethsv(hue_value[i], 255, value[i], (rgb_led_t *)&led[i]);
        //}else{
        //    sethsv(rgblight_config.hue, hue_value[i], value[i], (rgb_led_t *)&led[i]); 
        //}
    }
    underled(true,0,0);
    rgblight_set();
    rgblight_value(0,0,false,true,false); //軌跡削除処理
}
...
void rgblight_effect_static(void) {
    for (uint8_t i = 0; i < RGBLED_NUM; i++) {
        sethsv(rgblight_config.hue, 255, rgblight_config.val, (rgb_led_t *)&led[i]);
    }
    rgblight_set();
}
