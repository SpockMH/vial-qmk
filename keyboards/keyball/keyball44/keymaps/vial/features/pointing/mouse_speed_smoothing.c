...
// マウスレポート1個を渡すだけで push→apply(x)→apply(y) をまとめて行うラッパー。
// pointing_device_task_user側の呼び出しを1行にするために追加。
void mouse_speed_smoothing(report_mouse_t *r) {
    int16_t abs_move = (r->x < 0 ? -r->x : r->x) + (r->y < 0 ? -r->y : r->y);
    mouse_speed_smoothing_push(abs_move);
    r->x = mouse_speed_smoothing_apply(r->x);
    r->y = mouse_speed_smoothing_apply(r->y);
}
