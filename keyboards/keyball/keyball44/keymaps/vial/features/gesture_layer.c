...
// 支配軸(絶対値が大きい方)だけ1回発火させる
static void gesture_fire(uint16_t up, uint16_t down, uint16_t left, uint16_t right) {
    ...
    if (ax >= ay) {
        tap_code16(tension_x > 0 ? right : left);
    } else {
        tap_code16(tension_y > 0 ? down : up);
    }
}

// 支配軸(絶対値が大きい方)だけ1回発火させる
static void gesture_fire_func(void (*on_up)(void), void (*on_down)(void),
    ...
    switch (state) {
        ...
        case GESTURE_STATE_MOVING:
            ...
            if (TIMER_DIFF_32(now, motion_start_time) >= GESTURE_FIRE_DELAY_MS) {
                gesture_fire(up, down, left, right);
...
void process_gesture_layer_func(int16_t x, int16_t y, 
    ...
    switch (state) {
        ...
        case GESTURE_STATE_MOVING:
            ...
            // 動いていようが止まっていようが、50ms経過したら判定・発火
            if (TIMER_DIFF_32(now, motion_start_time) >= GESTURE_FIRE_DELAY_MS) {
                gesture_fire_func(on_up, on_down, on_left, on_right);
                state = GESTURE_STATE_LOCKED;
                last_motion_time = now; // ここから10ms停止検知を開始
            }
            break;
...
void process_gesture_layer_key(int16_t x, int16_t y) {
    bool     has_motion = (x != 0 || y != 0);
    uint32_t now        = timer_read32();
